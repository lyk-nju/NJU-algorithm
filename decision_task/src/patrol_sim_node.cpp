...
namespace patrol_sim
{

class PatrolSimNode : public rclcpp::Node
{
public:
        ...
        : Node("patrol_sim_node")
    {
        ...
        autoaim_sub_ = this->create_subscription<std_msgs::msg::Float32MultiArray>(
            "/autoaim_data", qos,
            std::bind(&PatrolSimNode::autoAimCallback, this, std::placeholders::_1));

        nav_feedback_sub_ = this->create_subscription<NavigateThroughPoses::Impl::FeedbackMessage>(
            nav_feedback, qos,
            std::bind(&PatrolSimNode::navFeedbackCallback, this, std::placeholders::_1));
        nav_status_sub_ = this->create_subscription<action_msgs::msg::GoalStatusArray>(
            nav_status, qos,
            std::bind(&PatrolSimNode::navStatusCallback, this, std::placeholders::_1));
        ...
        RCLCPP_INFO(this->get_logger(), "PatrolSimNode initialized with %zu waypoints", rdsys_->wayPointMap.size());

        timer_ = this->create_wall_timer(
            std::chrono::duration<double>(0.5),
            std::bind(&PatrolSimNode::checkAndSendGoal, this));
    }
...
private:
    ...
    void autoAimCallback(const std_msgs::msg::Float32MultiArray::SharedPtr msg)
    {
        ...
        bool cmd_valid = (msg->data[0] != 0.0f);

        if (cmd_valid && !has_enemy_)
        {
            has_enemy_ = true;
            RCLCPP_WARN(this->get_logger(), "Enemy detected! Cancelling navigation...");
            cancelNavGoal();
        }
        else if (!cmd_valid && has_enemy_)
        {
            has_enemy_ = false;
            RCLCPP_INFO(this->get_logger(), "Enemy lost! Resuming patrol...");
        }
    }
    ...
    void navStatusCallback(const action_msgs::msg::GoalStatusArray::SharedPtr msg)
    {
        ...
        if (!msg->status_list.empty())
        {
            ...
            {
                RCLCPP_INFO(this->get_logger(), "Goal reached! x=%.2f, y=%.2f", current_x_, current_y_);
    ...
    void checkAndSendGoal()
    {
        ...
        if (next_target < 0)
        {
            ...
            {
                next_target = 1;
                RCLCPP_INFO(this->get_logger(), "Starting from waypoint %d", next_target);
            }
            else
            {
                return;
    ...
    int findNextTarget(int current_wp_id)
    {
        for (const auto& dec : rdsys_->decisions)
        {
            ...
            {
                RCLCPP_INFO(this->get_logger(), "Decision: %s -> waypoint %d",
...
int main(int argc, char** argv)
{
    ...
    auto node = std::make_shared<patrol_sim::PatrolSimNode>();
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}
