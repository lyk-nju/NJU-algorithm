...
void launch_preprocess_kernel(
    const uint8_t *src, int src_w, int src_h, int src_step, // 输入：原始图像
...
{
  public:
    // 默认加载 video_test 使用的 TensorRT 引擎
    Detector();
  ...
  private:
    ...
    float scale_; // 缩放比例
    float pad_x_; // 在 640x640 上的 x 方向 padding
    float pad_y_; // 在 640x640 上的 y 方向 padding
