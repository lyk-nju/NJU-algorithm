...
class PnpSolver
{
  public:
    ...
    // 切换当前相机参数（内参 + 外参 + distortion）。多相机场景下每帧由
    // AimPipeline 根据 FrameBundle.camera 调用；单相机场景下 AutoAimSystem
    // 也会每帧调一次，数值等于构造时从 yaml 读入的那一组——因此新增此接口
    // 不改变现行行为。
    //
    // 快速路径：若本次 info 的 camera_id 与对象地址都与上次相同，直接 return，
    // 不做任何复制。单相机场景下 AutoAimSystem::camera_info_ 是稳定的成员对象，
    // 每帧都命中快速路径，零开销。切换相机或遇到新的 CameraInfo 对象时才
    // clone 一次 cv::Mat，避免 cv::Mat shallow-copy 踩到外部对象销毁/改写
    // 的悬空风险。
    //
    // 契约：info.camera_matrix / info.distort_coeffs 必须非空；否则会让
    // solve_pnp 崩在 cv::solvePnP 上。AutoAimSystem::start() 负责从
    ...
    // 把当前相机参数打包成 CameraInfo 导出。Step A 过渡期：上层可以把
    // 构造期从 yaml 读入的那组参数直接拿出来填 FrameBundle.camera，避免
    // 重复解析 yaml 带来的潜在差异。Step C 接多相机后，上层会直接从
    // 配置构造 CameraInfo，此 getter 仍然可用于调试/fallback。
  ...
  private:
    ...
    // set_camera 的 fast-path 缓存键：
    //   - last_camera_id_ == -1 表示"还没被 set_camera 调过"（构造期直接读 yaml 的状态）。
    //   - last_info_ptr_ 记录上次传入 CameraInfo 的对象地址；即使 camera_id 相同，
    //     对象地址不同也视为换了一份，要重刷（防御"同 id 不同对象且内容被改过"的场景）。
    //   - 同时命中 id + 地址才跳过复制。
