...
{
  public:
    // 不变量：??aim(targets, ...) 被调用且 targets 非空时，
    //        debug_aim_point 保证在返回前被赋值为本次调用选中的瞄准点
    //        （无论成??失败——失败时 .valid 会被置为 false）??
    //        targets 为空这条早退分支不写该字段，但上??AimPipeline
    //        ??targets 为空时不会调??aim，所以外部观察到的永远是"本次调用的结????
    //        AimPipeline::step_impl ??DebugSnapshot 路径依赖这一不变量??
