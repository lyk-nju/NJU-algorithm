...
Aimer::Aimer(const std::string &config_path)
{
    ...
    yaw_offset_ = yaml["yaw_offset"].as<double>() / 57.3;       // 角度转弧??
    pitch_offset_ = yaml["pitch_offset"].as<double>() / 57.3;   // 角度转弧??
    // 兼容旧配置文件：优先??coming_angle，回退到历史拼??comming_angle
    if (yaml["coming_angle"])
        coming_angle_ = yaml["coming_angle"].as<double>() / 57.3; // 角度转弧??
    else
        coming_angle_ = yaml["comming_angle"].as<double>() / 57.3; // 角度转弧??
    leaving_angle_ = yaml["leaving_angle"].as<double>() / 57.3; // 角度转弧??
